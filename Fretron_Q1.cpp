...
void drawFlightPath(SDL_Renderer* renderer, const std::vector<Point>& flightPath, SDL_Color color) {
    SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, color.a);
    for (size_t i = 0; i < flightPath.size() - 1; ++i) {
        SDL_RenderDrawLine(renderer, flightPath[i].x * 20, flightPath[i].y * 20, flightPath[i + 1].x * 20, flightPath[i + 1].y * 20);
    }
}
...
int main() {
    std::vector<Point> flight1 = { Point(1, 1), Point(2, 2), Point(3, 3) };
    std::vector<Point> flight2 = { Point(1, 1), Point(2, 4), Point(3, 2) };
    std::vector<Point> flight3 = { Point(1, 1), Point(4, 2), Point(3, 4) };
    ...
    drawFlightPath(renderer, flight1, blue);
    drawFlightPath(renderer, flight2, green);
    drawFlightPath(renderer, flight3, red);
